...
def get_personal_results(df_callibration,df_test,target_var,output_dir,model_type,calibration_subjects):
    
    feat = df_callibration.columns.values[:-3]
    ...
    for subject in calibration_subjects:
        ...
        result.to_csv(os.path.join(output_dir, "subject_" + str(int(subject)) + ".csv"), index=True)
